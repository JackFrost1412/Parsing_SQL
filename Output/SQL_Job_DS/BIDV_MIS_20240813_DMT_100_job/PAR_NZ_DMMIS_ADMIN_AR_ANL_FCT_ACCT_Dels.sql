SELECT 
       ...
       , NVL(TM_PRD.CDR_DT_DIM_ID, '999999')   		TM_PRD_DIM_ID  
       , TWT.TOT_AVL_AMT_FCY
       , TWT.TOT_AVL_AMT_LCY
       ...
       , NVL(TWT.TOT_LMT_AMT_FCY, 0)		TOT_LMT_AMT_FCY
       , NVL(TWT.TOT_LMT_AMT_LCY, 0)		TOT_LMT_AMT_LCY
	   ...
	   , NVL(MTH_ISSU_TP_DIM.MTH_ISSU_TP_DIM_ID, 999999)		MTH_ISSU_TP_DIM_ID   
       , NVL(RTO_DBT_TP_DIM.RTO_DBT_TP_DIM_ID, 999999)			RTO_DBT_TP_DIM_ID
       ...
       , NVL(EMPE_CREATE_AR.EMPE_DIM_ID, 999999)   				AR_OFCR_CREATOR_DIM_ID 		       , NVL(TMP_CST.CST_OFCR_CREATOR_DIM_ID, 999999)   			CST_OFCR_CREATOR_DIM_ID		        , NVL(AR_LCS_DT.CDR_DT_DIM_ID, '999999')					AR_LCS_DT_DIM_ID    	   , NVL(TMP.CST_LBY_BAL_SEG_DIM_ID,  999999)							CST_LBY_BAL_SEG_DIM_ID 	   , NVL(TMP.CST_AST_BAL_SEG_DIM_ID, 999999)							CST_AST_BAL_SEG_DIM_ID 	   
...
,        NVL(TWT.TOT_LMT_AMT_FCY, 0)		LMT_AMT_FCY
       , NVL(TWT.TOT_LMT_AMT_LCY, 0)		LMT_AMT_LCY
...
LEFT JOIN 	MTH_ISSU_TP_DIM MTH_ISSU_TP_DIM		ON MTH_ISSU_TP_DIM.LVL_0_ID = TWT.MTH_ISSU_TP_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= MTH_ISSU_TP_DIM.EFF_FM_DT AND MTH_ISSU_TP_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') 
LEFT JOIN 	RTO_DBT_TP_DIM RTO_DBT_TP_DIM		ON RTO_DBT_TP_DIM.LVL_0_ID = TWT.RTO_DBT_TP_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= RTO_DBT_TP_DIM.EFF_FM_DT AND RTO_DBT_TP_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') 
...
LEFT JOIN CDR_DT_DIM TM_PRD				ON TM_PRD.CDR_DT_DIM_ID = TWT.TM_PRD_ID
LEFT JOIN CDR_DT_DIM AR_LCS_DT	  ON TWT.AR_LCS_DT_ID = AR_LCS_DT.CDR_DT_DIM_ID 
...
LEFT JOIN AR_FNC_ST_DIM  AR_FNC_ST_DIM	ON TWT.PAST_DUE_PNP_NBR_OF_DYS >= AR_FNC_ST_DIM.MIN_DAY AND TWT.PAST_DUE_PNP_NBR_OF_DYS < AR_FNC_ST_DIM.MAX_DAY
	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= AR_FNC_ST_DIM.EFF_FM_DT AND AR_FNC_ST_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
...
LEFT JOIN 		TWT_CST_LMT_SEG TWT_LMT_SEG			ON TWT_LMT_SEG.CST_ID = TWT.CST_ID AND TWT_LMT_SEG.MSR_PRD_ID   =  TM_PRD.CDR_DT_DIM_ID
...
LEFT JOIN 	CSTB_AR_TM_TO_MAT_SEQ_MP_RL ORG_TERM_SEG ON ORG_TERM_SEG.TERM_CODE = 'D' 
		AND TWT.ORIG_TM_TO_MAT_SEG_ID >= ORG_TERM_SEG.MIN_TERM AND TWT.ORIG_TM_TO_MAT_SEG_ID < ORG_TERM_SEG.MAX_TERM 
...
WHERE AR_DIM.SRC_STM = 'CADENCIE_ACCT' AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= AR_DIM.EFF_FM_DT AND AR_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
;
