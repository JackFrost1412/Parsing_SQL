SELECT NVL(t21.EMPE_DIM_ID, 999999)	AR_RLTNP_OFCR_DIM_ID
       ...
       , NVL(t19.CNL_PVDR_DIM_ID, 999999)	CNL_TP_PVDR_DIM_ID       , NVL(ft.FEE_TP_DIM_ID, 999999)		FEE_TP_DIM_ID       , m.TXN_FEE_EXPN_AMT_FCY	TXN_FEE_EXPN_AMT_FCY
       , m.TXN_FEE_EXPN_AMT_LCY	TXN_FEE_EXPN_AMT_LCY
       , m.TXN_FEE_INCM_AMT_FCY	TXN_FEE_INCM_AMT_FCY
       , m.TXN_FEE_INCM_AMT_LCY	TXN_FEE_INCM_AMT_LCY
	   ...
	   , NVL(OU_DIM.ORG_UNIT_DIM_ID, 999999) TXN_ORG_DIM_UNIT_ID
		...
		, NVL(RQS_CNL_MGT_DEP.ORG_UNIT_DIM_ID, 999999)	RQS_CNL_MGT_DEP_DIM_ID   FROM TWT_FEE_CNL_TXN_ANL_FCT m
 INNER JOIN TWT_CNL_TXN_ANL_FCT t ON t.TXN_ID = m.TXN_ID
 
  INNER JOIN CSTB_SYSTEM SYS ON SYS.PARAM_NAME = 'ETL_DATE'
 LEFT JOIN CNL_DIM t2 ON t2.CNL_ID = t.CNL_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t2.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t2.EFF_TO_DT
 LEFT JOIN CNL_TP_DIM t3 ON t3.LVL_0_ID = t.CNL_TP_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t3.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t3.EFF_TO_DT
 LEFT JOIN CCY_DIM t4 ON t4.CCY_ID = t.CCY_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t4.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t4.EFF_TO_DT
 LEFT JOIN CST_DIM t5 ON t5.CST_ID = t.CST_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t5.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t5.EFF_TO_DT
 LEFT JOIN MRCH_DIM t9 ON t9.MRCH_ID = t.MRCH_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t9.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t9.EFF_TO_DT
 LEFT JOIN AR_DIM t11 ON t11.AR_ID = t.PRIM_AR_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t11.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t11.EFF_TO_DT
 LEFT JOIN OU_DIM t12 ON t12.ORG_UNIT_ID = t.PRIM_ORG_DIM_UNIT_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t12.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t12.EFF_TO_DT
 LEFT JOIN PD_DIM t13 ON t13.PD_ID = t.PD_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t13.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t13.EFF_TO_DT
 LEFT JOIN CNL_DIM t14 ON t14.CNL_ID = t.SETL_CNL_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t14.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t14.EFF_TO_DT
 LEFT JOIN TXN_CODE_DIM t15 ON t15.LVL_0_ID = t.TXN_CODE_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t15.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t15.EFF_TO_DT
 LEFT JOIN TXN_LC_ST_DIM t16 ON t16.LVL_0_ID = t.TXN_LIFE_CYC_ST_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t16.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t16.EFF_TO_DT
 LEFT JOIN TXN_NTW_DIM t17 ON t17.LVL_0_ID = t.TXN_NTW_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t17.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t17.EFF_TO_DT
 LEFT JOIN MRCH_CGY_DIM t18 ON t18.LVL_0_ID = t.MRCH_CGY_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t18.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t18.EFF_TO_DT
 LEFT JOIN CNL_TP_PVDR_DIM t19 ON t19.LVL_0_ID = t.CNL_TP_PVDR_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t19.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t19.EFF_TO_DT
 LEFT JOIN EMPE_DIM t21 ON t21.EMPE_ID = t.AR_RLTNP_OFCR_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t21.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t21.EFF_TO_DT
 LEFT JOIN SRC_STM_DIM t24 ON t24.LVL_0_ID = t.SRC_STM_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t24.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t24.EFF_TO_DT
 LEFT JOIN CTY_DIM t25 ON t25.LVL_0_ID = t.CTY_OF_TXN_DIM_ID
 	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t25.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t25.EFF_TO_DT
   
  
 LEFT JOIN FEE_TP_DIM ft ON ft.LVL_0_ID = m.FEE_TP_DIM_ID 
...
LEFT JOIN (SELECT DISTINCT PD_ID, CARD_TP_ACQ_LVL_0_ID FROM EX_MAP_CARD_TP_ACQ e
            ...
            AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= e.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < e.EFF_TO_DT
            ) e ON e.PD_ID = t.PD_DIM_ID
LEFT JOIN CARD_TP_ACQ_DIM t26 ON t26.LVL_0_ID = e.CARD_TP_ACQ_LVL_0_ID
	AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t26.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t26.EFF_TO_DT
LEFT JOIN AR_DIM_SVC t27 ON t27.AR_ID = t.PRIM_AR_DIM_ID
    AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= t27.EFF_FM_DT AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') < t27.EFF_TO_DT
...
LEFT JOIN EMPE_DIM   on EMPE_DIM.EMPE_ID=t.OBJ_IP_ID 
	AND TO_DATE(sys.param_value,'YYYYDDD') >= EMPE_DIM.EFF_FM_DT
	AND EMPE_DIM.EFF_TO_DT >  TO_DATE(sys.param_value,'YYYYDDD')
LEFT JOIN OU_DIM ON T.TXN_ORG_UNIT_ID = OU_DIM.ORG_UNIT_ID
	AND OU_DIM.EFF_FM_DT  <= TO_DATE(sys.param_value,'YYYYDDD') 
	AND OU_DIM.EFF_TO_DT > TO_DATE(sys.param_value,'YYYYDDD')
	
LEFT JOIN OU_DIM t30 ON t30.ORG_UNIT_ID = t.MRCH_MGT_DEP_DIM_ID
 	AND TO_DATE(sys.param_value,'YYYYDDD') >= t30.EFF_FM_DT AND TO_DATE(sys.param_value,'YYYYDDD') < t30.EFF_TO_DT
LEFT JOIN OU_DIM t31 ON t31.ORG_UNIT_ID = t.CNL_MGT_DEP_DIM_ID
 	AND TO_DATE(sys.param_value,'YYYYDDD') >= t31.EFF_FM_DT AND TO_DATE(sys.param_value,'YYYYDDD') < t31.EFF_TO_DT
LEFT JOIN OU_DIM t32 ON t32.ORG_UNIT_ID = t.AR_MGT_DEP_DIM_ID
 	AND TO_DATE(sys.param_value,'YYYYDDD') >= t32.EFF_FM_DT AND TO_DATE(sys.param_value,'YYYYDDD') < t32.EFF_TO_DT
LEFT JOIN EMPE_DIM T33 ON T33.EMPE_ID=T.CNL_OFCR_EMPE_DIM_ID
 	AND TO_DATE(sys.param_value,'YYYYDDD') >= t32.EFF_FM_DT AND TO_DATE(sys.param_value,'YYYYDDD') < t32.EFF_TO_DT

LEFT JOIN OU_DIM RQS_CNL_MGT_DEP ON RQS_CNL_MGT_DEP.ORG_UNIT_ID = t.RQS_CNL_MGT_DEP_ID AND TO_DATE(SYS.PARAM_VALUE, 'YYYYDDD') BETWEEN RQS_CNL_MGT_DEP.EFF_FM_DT AND RQS_CNL_MGT_DEP.EFF_TO_DT - 1 --FSS BO SUNG MIS2-GD4 NGAY 15/08/2019;
