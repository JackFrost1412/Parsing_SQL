...
insert into AST_AR_ANL_FCT_SMY
SELECT ACG_STC_GRP_DIM_ID
       ...
       , sum(CLS_BAL_AMT_LMTH_FCY) CLS_ON_BAL_AMT_LMTH_FCY
       , sum(CLS_BAL_AMT_LMTH_LCY) CLS_ON_BAL_AMT_LMTH_LCY
       , sum(CLS_BAL_AMT_LQTR_FCY) CLS_ON_BAL_AMT_LQTR_FCY
       , sum(CLS_BAL_AMT_LQTR_LCY) CLS_ON_BAL_AMT_LQTR_LCY
       , sum(CLS_BAL_AMT_LYR_FCY) CLS_ON_BAL_AMT_LYR_FCY
       , sum(CLS_BAL_AMT_LYR_LCY) CLS_ON_BAL_AMT_LYR_LCY
	   , sum(case when src.LVL_0_CODE in ('SIBS_LNMAST', 'SIBS_ODTIER', 'CADENCIE_ACCT','SGD3_TCNT_CHOVAY','SGD3_ODA_DUNOUT') then T.CLS_BAL_AMT_TDY_FCY else 0 end) CLS_ON_BAL_AMT_TDY_FCY
	   , sum(case when src.LVL_0_CODE in ('SIBS_LNMAST', 'SIBS_ODTIER', 'CADENCIE_ACCT','SGD3_TCNT_CHOVAY','SGD3_ODA_DUNOUT') then T.CLS_BAL_AMT_TDY_LCY else 0 end) CLS_ON_BAL_AMT_TDY_LCY
		
       , sum(CLS_BAL_AMT_LAST_DAY_FCY) CLS_ON_BAL_AMT_PREV_FCY
       , sum(CLS_BAL_AMT_LAST_DAY_LCY) CLS_ON_BAL_AMT_PREV_LCY
 	   , sum(case when pd.PD_CODE in ('TFLC01', 'TFUB01') then t.CLS_BAL_AMT_TDY_FCY else 0 end) CLS_LC_BAL_AMT_TDY_FCY
	   , sum(case when pd.PD_CODE in ('TFLC01', 'TFUB01') then t.CLS_BAL_AMT_TDY_LCY else 0 end) CLS_LC_BAL_AMT_TDY_LCY

	   , sum(case when pd.PD_CODE like 'BG%' then t.CLS_BAL_AMT_TDY_FCY else 0 end) CLS_BG_BAL_AMT_TDY_FCY
	   , sum(case when pd.PD_CODE like 'BG%' then t.CLS_BAL_AMT_TDY_LCY else 0 end) CLS_BG_BAL_AMT_TDY_LCY
