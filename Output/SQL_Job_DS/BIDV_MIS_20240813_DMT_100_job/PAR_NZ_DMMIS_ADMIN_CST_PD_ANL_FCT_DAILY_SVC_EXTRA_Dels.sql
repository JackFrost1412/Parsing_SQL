...
LEFT JOIN 		(
		...
		LEFT JOIN (				
					SELECT 
							...
							PD_ID		
							, ORG_UNIT_ID
							, max(LAST_CST_TXN_DT_DIM_ID)    LAST_CST_TXN_DT_DIM_ID
							, S.LVL_0_ID	SRC_STM_ID
					FROM CST_PD_ANL_FCT_PREDAY F
					LEFT JOIN CSTB_SYSTEM SYS ON SYS.PARAM_NAME = 'ETL_DATE'
					INNER JOIN SRC_STM_DIM S ON S.SRC_STM_DIM_ID = F.SRC_STM_DIM_ID 									group by cst_id, pd_id, org_unit_id, S.LVL_0_ID
		) FCT_PRE ON  FCT_PRE.CST_ID = TWT.CST_ID AND FCT_PRE.PD_ID = TWT.PD_ID AND FCT_PRE.ORG_UNIT_ID = TWT.ORG_UNIT_ID  AND FCT_PRE.SRC_STM_ID = twt.src_stm_id  
...
LEFT JOIN (SELECT 
			...
			, S.LVL_0_ID	SRC_STM_ID
	FROM CST_PD_ANL_FCT_PREDAY F
	INNER JOIN SRC_STM_DIM S ON S.SRC_STM_DIM_ID = F.SRC_STM_DIM_ID ) PRE  ON  PRE.CST_ID = TWT.CST_ID    AND PRE.PD_ID = TWT.PD_ID AND PRE.ORG_UNIT_ID = TWT.ORG_UNIT_ID AND PRE.SRC_STM_ID = TWT.SRC_STM_ID 
