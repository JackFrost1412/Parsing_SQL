SELECT 
       ...
       , NVL(TMP_CST_SEG.CST_LIFE_CYC_ST_DIM_ID, '999999')			CST_LCS_TP_DIM_ID
	...
	, NVL(TWT.TOT_NBR_OF_CST_TXN_TDY, 0)						TOT_NBR_OF_CST_TXN_TDY
       ...
       , twt.TOT_NBR_OF_PASSIVE_TXN_DB_TDY		TOT_NBR_OF_PASSIVE_TXN_DB_TDY	
		...
		, CASE WHEN TWT.TOT_NBR_OF_CST_TXN_TDY > 0 THEN	 TO_NUMBER(CDR_DT_DIM.CDR_DT_DIM_ID, 9999999)  ELSE PRE.LAST_CST_TXN_DT_DIM_ID END LAST_CST_TXN_DT_DIM_ID		
		, NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0)			TOT_AST_ACR_INT_AMT_TDY_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_MO = 1	THEN NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0) ELSE  NVL(PRE.TOT_AST_ACR_INT_AMT_MTD_LCY, 0) + NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0) END			TOT_AST_ACR_INT_AMT_MTD_LCY
		, CASE WHEN TO_NUMBER(extract (day from TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') - date_trunc('quarter', TO_DATE(SYS.PARAM_VALUE,'YYYYDDD'))),99) + 1 = 1 
	      THEN NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0)   ELSE NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0) + NVL(PRE.TOT_AST_ACR_INT_AMT_QTD_LCY, 0)  END 		TOT_AST_ACR_INT_AMT_QTD_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_YR = 1	THEN NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0) ELSE 	NVL(TWT.TOT_AST_ACR_INT_AMT_TDY_LCY, 0) + 	NVL(PRE.TOT_AST_ACR_INT_AMT_YTD_LCY, 0)  END 		TOT_AST_ACR_INT_AMT_YTD_LCY

		, NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0)			TOT_LBY_ACR_INT_AMT_TDY_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_MO = 1	THEN NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0) ELSE  NVL(PRE.TOT_LBY_ACR_INT_AMT_MTD_LCY, 0) + NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0) END			TOT_LBY_ACR_INT_AMT_MTD_LCY
		, CASE WHEN TO_NUMBER(extract (day from TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') - date_trunc('quarter', TO_DATE(SYS.PARAM_VALUE,'YYYYDDD'))),99) + 1 = 1 
	      THEN NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0)   ELSE NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0) + NVL(PRE.TOT_LBY_ACR_INT_AMT_QTD_LCY, 0)  END 		TOT_LBY_ACR_INT_AMT_QTD_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_YR = 1	THEN NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0) ELSE 	NVL(TWT.TOT_LBY_ACR_INT_AMT_TDY_LCY, 0) + 	NVL(PRE.TOT_LBY_ACR_INT_AMT_YTD_LCY, 0)  END 		TOT_LBY_ACR_INT_AMT_YTD_LCY

		, NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0)			TOT_AST_INT_PYMT_AMT_TDY_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_MO = 1	THEN NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0) ELSE  NVL(PRE.TOT_AST_INT_PYMT_AMT_MTD_LCY, 0) + NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0) END			TOT_AST_INT_PYMT_AMT_MTD_LCY
		, CASE WHEN TO_NUMBER(extract (day from TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') - date_trunc('quarter', TO_DATE(SYS.PARAM_VALUE,'YYYYDDD'))),99) + 1 = 1 
	      THEN NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0)   ELSE NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0) + NVL(PRE.TOT_AST_INT_PYMT_AMT_QTD_LCY, 0)  END 		TOT_AST_INT_PYMT_AMT_QTD_LCY
		, CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_YR = 1	THEN NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0) ELSE 	NVL(TWT.TOT_AST_INT_PYMT_AMT_TDY_LCY, 0) + 	NVL(PRE.TOT_AST_INT_PYMT_AMT_YTD_LCY, 0)  END 		TOT_AST_INT_PYMT_AMT_YTD_LCY
		, 0 	NBR_OF_OD_AC
		, 'N' 	SLRY_CR_TXN_IND
...
LEFT JOIN (SELECT 
	...
	INNER JOIN SRC_STM_DIM ON SRC_STM_DIM.SRC_STM_DIM_ID = F.SRC_STM_DIM_ID AND SRC_STM_DIM.LVL_0_CODE IN ('DATATM_X1PCMS')
...
LEFT JOIN 		(
			SELECT 
				...
				TWT.ORG_UNIT_ID,
				TWT.SRC_STM_ID
			FROM TWT_CST_PD_ANL_X1PCMS TWT
			INNER JOIN 		CSTB_SYSTEM SYS ON SYS.PARAM_NAME = 'ETL_DATE'
			LEFT JOIN SRC_STM_DIM SRC_STM_DIM	ON SRC_STM_DIM.LVL_0_ID	= TWT.SRC_STM_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= SRC_STM_DIM.EFF_FM_DT AND SRC_STM_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
			LEFT JOIN CST_PD_ANL_FCT_PREDAY FCT_PRE ON  FCT_PRE.CST_ID = TWT.CST_ID AND FCT_PRE.PD_ID = TWT.PD_ID AND FCT_PRE.ORG_UNIT_ID = TWT.ORG_UNIT_ID  AND FCT_PRE.SRC_STM_DIM_ID = SRC_STM_DIM.SRC_STM_DIM_ID 
