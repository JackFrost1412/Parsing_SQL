...
for index, row in df.iterrows():
    ...
    # DataFrame để lưu kết quả của từng loại extract
    extract_results = []
    
    if has_join == False:  # Nếu câu truy vấn không chứa JOIN
        # Gọi hàm extract_simple_sql
        simple_df = Extract_Simple_SQL.process_simple_query(query)
        extract_results.append((simple_df))
        
    else:  # Nếu có JOIN
        if query.strip().upper().startswith('WITH'):  # Kiểm tra nếu bắt đầu bằng WITH
            # Gọi hàm extract_with_cte
            cte_with_df = Extract_WITH_CTE.process_sql_query_to_dfs(query)
            extract_results.append((cte_with_df))
            print(extract_results)
    #     else:  # Nếu không bắt đầu bằng WITH
    #         # Gọi hàm extract_subselects
    #         table_df, column_df = Extract_Subselects.process_sql_with_source(query)
    #         extract_results.append((table_df, column_df))

    # # Sau khi xử lý xong từng query, union all các bảng
    # for table_df, column_df in extract_results:
    #     # Duyệt qua từng bảng alias và nối với thông tin cột tương ứng
    #     for _, table_row in table_df.iterrows():
    #         table_name = table_row['Table Name']
    #         table_alias = table_row['Table Alias']
