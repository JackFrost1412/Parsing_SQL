...
\(9)   , NVL(SLRY_TVR_SEG_DIM.SLRY_TVR_SEG_DIM_ID, 999999)\(9)\(9)\(9)SLRY_TVR_DIM_ID
     ...
     --  , CASE WHEN TWT.IP_TP = 'DN' THEN NVL(CST_MSEG_DN.CST_MKT_SEG_DIM_ID, 999999) ELSE NVL(CST_MSEG_CN.CST_MKT_SEG_DIM_ID, 999999)\(9)END\(9)\(9)\(9)\(9)\(9)CST_MKT_SEG_DIM_ID
...
\(9)   --T?ng s? du huy d?ng v?n t? khᣨ h஧ t?i th?i di?m cu?i thᮧ / cu?i qu𛰠cu?i nam tru?c.
       , CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_MO = 1 THEN NVL(PRE.TOT_LBY_CLS_BAL_AMT_TDY_LCY, 0) ELSE NVL(PRE.TOT_LBY_CLS_BAL_AMT_LMTH_LCY, 0)\(9)END     TOT_LBY_CLS_BAL_AMT_LMTH_LCY
       , CASE WHEN TO_NUMBER(extract (day from TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') - date_trunc('quarter', TO_DATE(SYS.PARAM_VALUE,'YYYYDDD'))),99) + 1 = 1
...
\(9)\(9), CASE WHEN TWT.TOT_NBR_OF_CST_TXN_TDY > 0 THEN\(9) TO_NUMBER(CDR_DT_DIM.CDR_DT_DIM_ID, 9999999)  ELSE PRE.LAST_CST_TXN_DT_DIM_ID END LAST_CST_TXN_DT_DIM_ID\(9)\(9)--LAY THEO SO LUONG GIAO DICH + JOIN PREDAY
...
FROM TWT_CST_PD_ANL_CADENCIE   TWT
...
LEFT JOIN SRC_STM_DIM SRC_STM_DIM\(9)ON SRC_STM_DIM.LVL_0_ID\(9)= TWT.SRC_STM_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= SRC_STM_DIM.EFF_FM_DT AND SRC_STM_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')

LEFT JOIN \(9)\(9)(
\(9)\(9)SELECT 
\(9)\(9)\(9)CASE WHEN TWT.CST_PD_ST_BRN_CODE  NOT IN ('CLOSE ALL', 'CLOSE ALL TODAY') THEN (
\(9)\(9)\(9)\(9)CASE \(9)WHEN NVL(TWT.TOT_AST_CLS_BAL_AMT_TDY_LCY, 0) + NVL(TWT.TOT_LBY_CLS_BAL_AMT_TDY_LCY, 0)  > 0 OR NVL(TWT.LAST_CST_TXN_DT_DIM_ID, FCT_PRE.LAST_CST_TXN_DT_DIM_ID) >= TO_NUMBER(TO_CHAR(add_months(TO_DATE(SYS.PARAM_VALUE, 'YYYYDDD'), -6), 'YYYYDDD'), 9999999) THEN 'USING'
\(9)\(9)\(9)\(9)\(9)\(9)WHEN NVL(TWT.LAST_CST_TXN_DT_DIM_ID, FCT_PRE.LAST_CST_TXN_DT_DIM_ID) IS NULL THEN 'NOT_USE'
\(9)\(9)\(9)\(9)\(9)\(9)ELSE 'DORMANT'
\(9)\(9)\(9)\(9)END)
\(9)\(9)\(9)ELSE 'UNKNOWN'
...
\(9)\(9)FROM TWT_CST_PD_ANL_CADENCIE TWT
...
INNER JOIN PD_DIM PD_DIM\(9)\(9)\(9)ON PD_DIM.PD_ID = TWT.PD_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= PD_DIM.EFF_FM_DT AND PD_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
...
LEFT JOIN \(9)\(9)OCP_DIM OCP_DIM\(9)\(9)\(9)\(9)ON OCP_DIM.LVL_0_ID = TWT.OCP_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= OCP_DIM.EFF_FM_DT AND OCP_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
...
LEFT JOIN \(9)\(9)CST_LC_ST_DIM CST_LCS_DIM   ON CST_LCS_DIM.LVL_0_ID = TWT.CST_LCS_TP_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= CST_LCS_DIM.EFF_FM_DT AND CST_LCS_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
--LEFT JOIN \(9)\(9)CST_MSEG_DIM CST_MKG_DIM\(9)ON CST_MKG_DIM.LVL_0_ID = TWT.CST_MKT_SEG_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= CST_MKG_DIM.EFF_FM_DT AND CST_MKG_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
LEFT JOIN \(9)\(9)IDY_CL_DIM IDY_CL_DIM\(9)    ON IDY_CL_DIM.LVL_0_ID = TWT.IDY_SML_ID_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= IDY_CL_DIM.EFF_FM_DT AND IDY_CL_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
LEFT JOIN \(9)\(9)MAR_ST_DIM MAR_ST_DIM\(9)\(9)ON MAR_ST_DIM.LVL_0_ID = TWT.MAR_ST_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= MAR_ST_DIM.EFF_FM_DT AND MAR_ST_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
LEFT JOIN \(9)\(9)EMPE_DIM CST_RLTNP_OFCR\(9)\(9)ON CST_RLTNP_OFCR.EMPE_ID = TWT.CST_RLTNP_OFCR_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= CST_RLTNP_OFCR.EFF_FM_DT AND CST_RLTNP_OFCR.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
LEFT JOIN \(9)\(9)CR_RSK_RTG_DIM  CR_RSK_RTG_DIM  ON CR_RSK_RTG_DIM.LVL_0_ID = TWT.CR_RSK_RTG_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= CR_RSK_RTG_DIM.EFF_FM_DT AND CR_RSK_RTG_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
LEFT JOIN \(9)\(9)PERF_ST_DIM PERF_ST_DIM\(9)\(9)ON PERF_ST_DIM.LVL_0_ID = TWT.CST_PERF_ST_ID AND TO_DATE(SYS.PARAM_VALUE,'YYYYDDD') >= PERF_ST_DIM.EFF_FM_DT AND PERF_ST_DIM.EFF_TO_DT >  TO_DATE(SYS.PARAM_VALUE,'YYYYDDD')
...
LEFT JOIN \(9)\(9)TMP_CST_SEG ON TMP_CST_SEG.CST_ID = TWT.CST_ID AND TMP_CST_SEG.TM_PRD_ID = TWT.TM_PRD_ID 
...
\(9)   \(9), NVL(TWT_TDY.CST_PD_ST_CODE, TWT.CST_PD_ST_CODE)\(9)\(9)CST_PD_ST_CODE
       , NVL(CST_PD_ST_DIM.CST_PD_ST_DIM_ID, 999999)\(9)\(9)CST_PD_ST_DIM_ID
\(9)   , TWT.CST_PD_ST_BRN_CODE\(9)\(9)CST_PD_ST_BRN_CODE
       , NVL(CST_PD_ST_BRN_DIM.CST_PD_ST_DIM_ID, 999999)\(9)\(9)CST_PD_ST_BRN_DIM_ID\(9)
...
\(9)     THEN NVL(TWT.TOT_NBR_OF_PASSIVE_TXN_DB_TDY , 0)  ELSE NVL(TWT.TOT_NBR_OF_PASSIVE_TXN_DB_TDY , 0)+ NVL(PRE.TOT_NBR_OF_PASSIVE_TXN_DB_QTD, 0) END \(9)TOT_NBR_OF_PASSIVE_TXN_DB_QTD
       , CASE WHEN CDR_DT_DIM.DAY_NBR_IN_CDR_YR = 1\(9)THEN NVL(TWT.TOT_NBR_OF_PASSIVE_TXN_DB_TDY , 0) ELSE NVL(TWT.TOT_NBR_OF_PASSIVE_TXN_DB_TDY , 0)+ NVL(PRE.TOT_NBR_OF_PASSIVE_TXN_DB_YTD, 0) END\(9)TOT_NBR_OF_PASSIVE_TXN_DB_YTD
       , twt.TOT_NBR_OF_PASSIVE_TXN_DB_TDY\(9)\(9)TOT_NBR_OF_PASSIVE_TXN_DB_TDY\(9)
\(9)\(9), CASE WHEN TWT_TDY.CLS_PD_IND = 1 THEN 'Y' ELSE 'N' END\(9)\(9)CLS_PD_IND
---- BACDT EDITED 10-10-2014
\(9)\(9), NVL(CTY_DIM.CTY_DIM_ID,'999999')\(9)\(9)\(9)\(9)\(9)CTY_OF_RSDNC_DIM_ID
\(9)\(9), NVL(EMPE_DIM.EMPE_DIM_ID, '999999')\(9)\(9)\(9)\(9)\(9)EMPE_DIM_ID
\(9)   
